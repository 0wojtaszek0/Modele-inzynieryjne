...
def oblicz_phi(t: np.ndarray, F: np.ndarray, S: np.ndarray,
    ...
    # 5) F_ref(i) - tylko do diagnostyki/wizualizacji (lokalizacja F_min)
    F_ref_t, valid = wyznacz_F_ref(t, F_filt, F_st, pairs, params)

    # 6) phi(i) = faza opóźnienia sygnału F(t) za z(t) wyznaczona Fourierowsko
    #
    # Metoda midpoint-między-przecięciami z normy SPECSUS2018 ma dwa problemy:
    #   (a) Wybiera "pierwsze rising" i "pierwsze falling" przecięcie z F_st
    #       w cyklu w byle jakim porządku, więc midpoint trafia w F_min lub
    #       F_max w zależności od fazy. To powoduje, że phi_raw daje to samo
    #       180° dla F w fazie z z(t) ORAZ F w antifazie - nie odróżnia tych
    #       skrajnych przypadków (analitycznie sprawdzone).
    #   (b) Składanie phi > 180° → 360° - phi tworzy V-kształt z minimum
    #       90° w rezonansie. phi_min w paśmie zawsze utyka koło 90°
    #       niezależnie od c_M (potwierdzone przez Sierackiego, który w
    #       pracy dyplomowej *omija* phi_min w 2DOF używając tylko EUSAMA).
    #
    # Definicja artykułu Wimad/EGEA (M. Klapka i in.; Kubiś, Wimad):
    #   "phi = kątowa różnica między sinusoidalnym przemieszczeniem płyty
    #    a sinusoidalnymi zmianami pionowej składowej siły kontaktu"
    #   "Wzrost tłumienia ZWIĘKSZA przesunięcie fazowe (tłumienie)"
    #   "minimalne phi ustali się powyżej 90°" dla dobrze stłumionego.
    #
    # To jest klasyczna definicja: phi = phase lag of F behind z, w zakresie
    # 0-360°, bez foldowania. Wyznaczamy ją projekcją sygnału F-F_st na bazę
    # (cos(theta), sin(theta)) w obrębie cyklu, gdzie theta=0 to chwila TOP_p:
    #   F-F_st = A*cos(theta) + B*sin(theta) = R*cos(theta - delta)
    #   delta = atan2(B, A)  → faza opóźnienia F za z(t)=d*cos(theta)
    # Wynik: phi_min monotonicznie rośnie z c_M (3° dla cM=100, 97° dla
    # cM=3000), zgodnie z normą EGEA i artykułem.
    ...
    for i, (a, b) in enumerate(pairs):
        ...
        if period <= 0:
            continue
        # Sprawdź, czy F-F_st ma wystarczającą amplitudę (margines wg params)
        delta_F = float(seg_F.max() - seg_F.min())
        if delta_F <= 0:
            continue
        Fst_lo = seg_F.min() + delta_F * params.RFstFMin / 100.0
        Fst_hi = seg_F.max() - delta_F * params.RFstFMax / 100.0
        ...
        theta = omega * (seg_t - TOPp[i])
        F_demean = seg_F - F_st
        A_F = 2.0 * np.mean(F_demean * np.cos(theta))
        B_F = 2.0 * np.mean(F_demean * np.sin(theta))
        delta_rad = np.arctan2(B_F, A_F)
        delta_deg_val = np.degrees(delta_rad)
        # Normalizacja do [0, 360): faza opóźnienia jest dodatnia
        if delta_deg_val < 0:
            delta_deg_val += 360.0
        phi_deg[i] = delta_deg_val
        valid[i] = True

    # 7) phi_min w zakresie pomiarowym (Min..MaxCalcFreq)
    #
    # UWAGA: rozbieg 0->25 Hz wg rozdz. 5.4 normy *również* krótko przechodzi
    # przez pasmo 6-18 Hz (cykle 1-11 w typowej symulacji). Te cykle są w
    # transjencie startowym i przy niskim tłumieniu dają spurious phi_min
    # (F(t) jeszcze nie ustabilizowane na wymuszeniu - dominuje swobodna
    # oscylacja koła ~13 Hz, F ma podwojoną częstotliwość względem z,
    # zwykłe wykrywanie F_ref daje błędne crossingi).
    #
    # Faza pomiarowa to *najdłuższy* ciąg cykli z f_period w paśmie - zjazd
    # 18->6 Hz trwający dTmeas=7.5 s (~90 cykli) wygrywa z krótkim rozbiegiem.
    in_band = (f_period >= params.MinCalcFreq) & (f_period <= params.MaxCalcFreq)
    meas_phase = np.zeros_like(in_band)
    if in_band.any():
        idx_in = np.where(in_band)[0]
        splits = np.where(np.diff(idx_in) > 1)[0]
        starts = np.concatenate(([0], splits + 1))
        ends = np.concatenate((splits, [len(idx_in) - 1]))
        longest = int(np.argmax(ends - starts + 1))
        meas_phase[idx_in[starts[longest]]:idx_in[ends[longest]] + 1] = True

    mask = meas_phase & valid & ~np.isnan(phi_deg)
    if mask.any():
    ...
    #    (używamy tej samej fazy pomiarowej co dla phi_min - patrz krok 7)
    idx_meas = np.where(meas_phase)[0]
    if idx_meas.size:
