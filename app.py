...
if tryb == "Symulacja 2DOF":
    ...
    if "sym_wyniki" in st.session_state:
        ...
        with tab1:
            ...
            # F_min do diagnozy bierzemy z fazy pomiarowej (przez `oblicz_phi`),
            # nie z calego sygnalu - inaczej transjent rozbiegu zaklamywal by
            # wykrycie odrywania kola (tak samo jak EUSAMA, patrz fix w modelu).
            F_min_val = float(analiza['F_min']) if np.isfinite(analiza['F_min']) \
                        else float(np.min(wyniki['F_t']))
        ...
        with tab5:
            ...
            # F_min raportu spojny z diagnoza i z trybem CSV - faza pomiarowa.
            F_min_raport = float(analiza['F_min']) if np.isfinite(analiza['F_min']) \
                           else float(np.min(wyniki['F_t']))
