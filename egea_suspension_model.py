...
class ModelCwiartkowy:
    ...
    def symuluj(self, fs: float = 10000.0) -> dict:
        ...
        # Wskaźnik EUSAMA: min F_t / F_st * 100 % w fazie pomiarowej.
        #
        # Maska f_step ∈ [6,18] łapie zarówno rozbieg 0→25 Hz (faza 1, ~0-2 s,
        # krótko przechodzi przez pasmo) jak i właściwą rampę zjazdu 18→6 Hz
        # (faza pomiarowa, ~5.8-13.3 s). Przy słabo tłumionych amortyzatorach
        # rozbieg generuje transjent z bardzo niskim F, co fałszuje EUSAMA.
        # Faza pomiarowa to *najdłuższy ciągły* odcinek w paśmie.
        in_band = (f_step >= self.egea.MinCalcFreq) & (f_step <= self.egea.MaxCalcFreq)
        if in_band.any():
            ...
            idx_meas = idx_in[starts[longest]:ends[longest] + 1]
